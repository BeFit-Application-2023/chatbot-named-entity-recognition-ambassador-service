...
from schemas import NERTextSchema, AwakeRequestSchema, IncreaseDecreaseSchema
...
def send_heartbeats():
    ...
    '''
    global ner_security_manager, SERVICE_NAME, SERVICE_HOST, SERVICE_KEY, SERVICE_PORT
    ...
    while True:
        ...
        service_hmac = ner_security_manager._SecurityManager__encode_hmac(test_json)

        try:
            # Sending the request.
            response = requests.get(
                f"http://{SERVICE_HOST}:{SERVICE_PORT}/ner",
                json = test_json,
        ...
        if status_code == 200 or status_code == 429:
            ...
            print("SEND HEARTBEAT TO SERVICE DICOVERY")
...
def awake():
    global ner_security_manager, SERVICE_NAME, SERVICE_HOST, SERVICE_KEY, SERVICE_PORT, CACHE_SERVICES, DATA_WAREHOUSE_DATA
    ...
    if check_response != "OK":
        ...
        if status_code != 200:
            ...
            ner_security_manager = SecurityManager(result["security"]["secret_key"])
...
def serve():
    global ner_security_manager, SERVICE_NAME, SERVICE_HOST, SERVICE_KEY, SERVICE_PORT, CACHE_SERVICES
    ...
    if check_response != "OK":
        ...
        # Validation of the json.
        result, status_code = ner_schema.validate_json(request.json)
        if status_code != 200:
            # If the request body didn't passed the json validation a error is returned.
            return result, status_code
        else:
            # Computing the Inference Service HMAC.
            service_hmac = ner_security_manager._SecurityManager__encode_hmac(result)
